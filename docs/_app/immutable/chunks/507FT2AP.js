import { o as w } from './Bfh70wKf.js';
import { d as c, g as u, b as f } from './rvO96ovo.js';
import { w as R } from './BOZwUnDS.js';
import { a as k } from './BPeLQb6m.js';
...
function H(t, e, n, r = !1) {
	const s = new URL(t);
	Object.defineProperty(s, 'searchParams', {
		value: new Proxy(s.searchParams, {
			get(a, o) {
				if (o === 'get' || o === 'getAll' || o === 'has') return (g) => (n(g), a[o](g));
				e();
				const l = Reflect.get(a, o);
				return typeof l == 'function' ? l.bind(a) : l;
			}
		}),
		enumerable: !0,
		configurable: !0
...
window.fetch = (t, e) => (
	(t instanceof Request ? t.method : e?.method || 'GET') !== 'GET' && h.delete(b(t)),
	x(t, e)
);
const h = new Map();
function K(t, e) {
	...
	if (r?.textContent) {
		...
		const a = r.getAttribute('data-ttl');
		return (
			a && h.set(n, { body: s, init: i, ttl: 1e3 * Number(a) }),
			r.getAttribute('data-b64') !== null && (s = O(s)),
...
function b(t, e) {
	...
	if (e?.headers || e?.body) {
		...
		(e.headers && s.push([...new Headers(e.headers)].join(',')),
			e.body && (typeof e.body == 'string' || ArrayBuffer.isView(e.body)) && s.push(e.body),
			(r += `[data-hash="${T(...s)}"]`));
	}
	return r;
}
const N = '1771184543246',
...
function te(t, e, n) {
	...
	const s = t instanceof SVGAElement ? t.target.baseVal : t.target,
		i = !r || !!s || I(r, e, n) || (t.getAttribute('rel') || '').split(/\s+/).includes('external'),
		a = r?.origin === S && t.hasAttribute('download');
	return { url: r, external: i, target: s, download: a };
}
function ne(t) {
	let e = null,
		n = null,
	...
	for (; o && o !== document.documentElement; )
		(r === null && (r = d(o, 'preload-code')),
			s === null && (s = d(o, 'preload-data')),
			e === null && (e = d(o, 'keepfocus')),
			n === null && (n = d(o, 'noscroll')),
			i === null && (i = d(o, 'reload')),
			a === null && (a = d(o, 'replacestate')),
			(o = A(o)));
	function l(g) {
		switch (g) {
			case '':
...
const E = { v: () => {} };
function se() {
	const { set: t, subscribe: e } = R(!1);
	let n;
	async function r() {
		clearTimeout(n);
		try {
			const s = await fetch(`${k}/_app/version.json`, {
				headers: { pragma: 'no-cache', 'cache-control': 'no-cache' }
			});
			if (!s.ok) return !1;
			const a = (await s.json()).version !== N;
			return (a && (t(!0), E.v(), clearTimeout(n)), a);
		} catch {
			return !1;
		}
	}
...
function I(t, e, n) {
	return t.origin !== S || !t.pathname.startsWith(e)
		? !0
		: n
...
function ae(t) {}
const U = new Set(['load', 'prerender', 'csr', 'ssr', 'trailingSlash', 'config']);
[...U];
const L = new Set([...U]);
[...L];
let m, y, p;
const P = w.toString().includes('$$') || /function \w+\(\) \{\}/.test(w.toString());
P
	? ((m = {
			data: {},
			form: null,
