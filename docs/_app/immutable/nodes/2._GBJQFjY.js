import { f as $, a as E } from '../chunks/CY5M9QH9.js';
import { o as R } from '../chunks/Bfh70wKf.js';
import {
	p as Z,
	c as o,
	s as i,
...
import { d as te } from '../chunks/BQJIW4hw.js';
import { h as oe } from '../chunks/Dyz6xL-l.js';
import { s as h } from '../chunks/BIZQoEtA.js';
import { s as ae } from '../chunks/MLHHVL6O.js';
import { s as ie } from '../chunks/DQ9wegHW.js';
import { b as re } from '../chunks/BrdBp72D.js';
import { b as p } from '../chunks/BPeLQb6m.js';
var ne = $(`<style>.home-bg {
			position: relative;
			isolation: isolate;
		}
...
function ge(W, Y) {
	...
	function F(e) {
		s(m) && (C = e.touches[0].clientX);
	}
	function A(e) {
		s(m) && ((_ = e.changedTouches[0].clientX), J());
	}
	function J() {
		const t = C - _;
		Math.abs(t) > 50 && (t > 0 ? l(c, !0) : l(c, !1));
	}
	function G() {
		s(m) && l(c, !s(c));
	}
	R(() => {
		const e = () => {
				(l(m, window.innerWidth <= 768), d && (f = d.offsetHeight));
			},
			t = () => {
				(clearTimeout(u), (u = window.setTimeout(e, 200)));
			};
		return (
			e(),
			window.addEventListener('resize', t),
			window.addEventListener('scroll', z, { passive: !0 }),
			setTimeout(() => {
				window.scrollY === 0 && window.scrollTo({ top: 1, behavior: 'smooth' });
			}, 2500),
			() => {
	...
	oe('1uha8ag', (e) => {
		var t = ne();
		(K(() => {
			Q.title = 'Operation Gospel - Abolish Abortion Georgia';
		}),
			E(e, t));
	});
	var n = o(v);
	((n.__touchstart = F), (n.__touchend = A));
	var g = o(n),
		x = o(g),
		M = o(x),
		w = i(o(M), 4),
	...
	(a(T),
		...
		U(() => {
			(ae(
				g,
				1,
				`hero-carousel ${s(c) ? 'show-text' : ''} md:block md:w-auto! md:transform-none! relative z-10 w-full px-6 md:px-16 mt-auto`
			),
				h(j, 'href', `${p ?? ''}/join`),
				h(O, 'href', `${p ?? ''}/support`),
				ie(y, `background-color: rgba(2, 6, 23, ${s(b) ?? ''}); backdrop-filter: blur(4px);`),
				h(B, 'href', `${p ?? ''}/georgia-battle`),
				h(V, 'href', `${p ?? ''}/abolition-basics`),
				h(D, 'href', `${p ?? ''}/join`));
		}),
		E(W, v),
		N());
}
te(['touchstart', 'touchend', 'click']);
