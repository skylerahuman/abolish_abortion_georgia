<script lang="ts">
	...
	onMount(() => {
		...
		const initMap = async () => {
			...
			// Optimization: Lazy load Leaflet JS and CSS
			const [L] = await Promise.all([
				import('leaflet'),
				import('leaflet/dist/leaflet.css')
			]);

			// Check if mapElement is still mounted after async import
			if (!mapElement) return;

			// Clean up existing map if any (shouldn't happen with observer logic, but safe)
			if (map) map.remove();
...
<div bind:this={mapElement} data-testid="map-container" class="h-full w-full rounded-xl border border-white/10 z-0"></div>
