<script lang="ts">
	...
	onMount(() => {
		...
		observer = new IntersectionObserver(
			(entries) => {
				// Optimization: Create a new Set to update state once per frame instead of per entry
				// But here we are updating visibleCards based on new entries.
				// For simplicity, we keep the logic but ensure we don't spam updates unnecessarily.
