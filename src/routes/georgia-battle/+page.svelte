<script lang="ts">
	...
	const timeline: TimelineEvent[] = timelineData as unknown as TimelineEvent[];
			...
			(entries) => {
				entries.forEach(entry => {
					if (entry.isIntersecting) {
						...
						if (id && !visibleCards.has(id)) {
							// Optimization: Create new Set only when adding a new item
							visibleCards = new Set(visibleCards).add(id);
						}
					}
				});
			},
